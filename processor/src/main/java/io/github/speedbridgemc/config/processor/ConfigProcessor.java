...
public final class ConfigProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element element : roundEnv.getElementsAnnotatedWith(Config.class)) {
            ...
            String handlerInterfaceIn = config.handlerInterface();
            if (handlerInterfaceIn.contains("."))
                handlerInterface = handlerInterfaceIn;
            else
                handlerInterface = configPackage + "." + handlerInterfaceIn;
            TypeName handlerInterfaceTypeName = TypeUtils.getTypeName(processingEnv, handlerInterface);
            if (handlerInterfaceTypeName == TypeName.VOID)
                continue;
            String handlerName;
            String[] handlerNameIn = config.handlerName();
            if (handlerNameIn.length == 0)
                handlerName = handlerInterface + "Impl";
            ...
            classBuilder.addSuperinterface(handlerInterfaceTypeName)
