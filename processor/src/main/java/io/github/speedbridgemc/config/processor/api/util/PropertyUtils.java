...
public final class PropertyUtils {
    ...
    public static final class AccessorInfo {
        ...
        public AccessorInfo(@NotNull Type type, @NotNull TypeMirror propertyType) {
            this.type = type;
    ...
    public static @NotNull Optional<AccessorInfo> getAccessorInfo(@NotNull ExecutableElement method) {
        if (method.getParameters().isEmpty() && method.getReturnType().getKind() != TypeKind.VOID)
            return Optional.of(new AccessorInfo(AccessorInfo.Type.GETTER, method.getReturnType()));
        else if (method.getParameters().size() == 1 && method.getReturnType().getKind() == TypeKind.VOID)
            return Optional.of(new AccessorInfo(AccessorInfo.Type.SETTER, method.getParameters().get(0).asType()));
